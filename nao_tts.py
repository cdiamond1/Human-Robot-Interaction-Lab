    ...
    try:
        ...
        if memory.getData("WordRecognized")[0] == wake_word:
            ...
            while get_turn() == "respond":
                ...
                # Wait until a new response is generated by checking the length or content
                while len(response) == len(prevResponce):
                ...
                print("len new response: " + str(len(response)) + " len prev response: " + str(len(prevResponce)))
                print("Speaking")
                animated_speech.say(response.encode('utf-8'))
                print("Assistant: " + response)

                # Update the previous response with the new one
